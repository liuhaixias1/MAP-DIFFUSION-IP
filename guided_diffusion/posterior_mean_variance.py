...
class MeanProcessor(ABC):
    ...
    @abstractmethod
    def __init__(self, betas, dynamic_threshold, clip_denoised, coffa, coffb):
...
class EpsilonXMeanProcessor(MeanProcessor):
    def __init__(self, betas, dynamic_threshold, clip_denoised, coffa, coffb):
        super().__init__(betas, dynamic_threshold, clip_denoised, coffa, coffb)
        alphas = 1.0 - betas
        alphas_cumprod = np.cumprod(alphas, axis=0)
        alphas_cumprod_prev = np.append(1.0, alphas_cumprod[:-1])
        ...
        # coffa = 1
        # coffb = 10
        aa = coffa * np.arange(len(betas))*betas
        bb = alphas_cumprod+coffb  
        # pa = 
        pb = np.sqrt(1 - alphas_cumprod) 
       
        self.sqrt_recip_alphas_cumprod1 = (np.sqrt(alphas_cumprod)+aa+coffb)/bb      
        self.sqrt_recipm1_alphas_cumprod1 = (pb+aa/pb) / bb
    ...
    def predict_xstart_map(self, x_t, t, eps):
        index = int(t)
        coef1 = self.sqrt_recip_alphas_cumprod1[index]
        coef2 = self.sqrt_recipm1_alphas_cumprod1[index]
