...
class GaussianDiffusion:
    def __init__(self,
        ...
        self.mean_processor = get_mean_processor(model_mean_type,
                                                 betas=betas,
                                                 dynamic_threshold=dynamic_threshold,
                                                 clip_denoised=clip_denoised, coffa = coffa, coffb = coffb)    
    ...
    def p_sample_loop_map(self,
        ...
        for idx in pbar:
            ...
            img = measurement_cond_fn(x_t=out['sample'],
                                      model=model,  # for pgdm, use kwargs to call it
                                      time=time,    # for pgdm, use kwargs to call it
                                      p_mean_variance = self.p_mean_variance,     # for pgdm, use kwargs to call it
                                      measurement=measurement,
                                      noisy_measurement=noisy_measurement,
                                      x_prev=img,
                                      x_0_hat=out['pred_xstart1'],
                                      H_funcs=H_funcs,
                                      noise_std =noise_std,
    ...
    def p_mean_variance(self, model, x, t):
        ...
        pred_xstart1 = self.mean_processor.predict_xstart_map(x, t, model_output)
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t):
        ...
        return {'sample': sample, 'pred_xstart': out['pred_xstart'], 'pred_xstart1': out['pred_xstart1']}
