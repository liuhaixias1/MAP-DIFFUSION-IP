...
class SuperResolution(H_functions):
    ...
    def Vt(self, vec):
        ...
        patches = torch.matmul(self.Vt_small, patches.reshape(-1, self.ratio ** 2, 1)).reshape(vec.shape[0],
                                                                                               self.channels, -1,
                                                                                               self.ratio ** 2)
        # print(patches.reshape(-1, self.ratio ** 2, 1).shape)    # [12288, 16, 1]
        # print(self.Vt_small.shape)
        # print(torch.matmul(self.Vt_small, patches.reshape(-1, self.ratio ** 2, 1)).shape)
        # print(patches.shape)    # [1, 3, 4096, 16]
    ...
    def singulars(self):
        # print(self.singulars_small.shape)
    ...
    # def HHt(self):
    #     return self.H_mat @ self.H_mat.t()
    #     # return self.U_small*self.singulars_small*self.singulars_small*self.U_small.t()
    #     # return self.ReturnH() * self.ReturnH().t()
    
    # def ReturnH(self):
    #     # print(self.y_dim)   # 64
    #     # print(self.ratio)   # 4
    #     # print(self.channels * self.y_dim ** 2 / self.ratio / self.ratio)  # 768.0
    #     # return torch.Tensor([[1 / self.ratio ** 2] * self.channels * self.y_dim ** 2]).to(self.H_mat.device)
    #     return self.H_mat
    
    # def H_NoShrink(self, vec):
    #     """
    #     Multiplies the input vector by H but no Shrink operation.
    #     """
    #     temp = self.Vt(vec)
    #     singulars = self.singulars()
